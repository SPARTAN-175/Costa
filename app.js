...
function ticketSvg(p,price,clientNote){
 const biz=db.business?.name||"Tu Negocio";
 const desc=clientNote?.trim()||p.description?.trim()||"Producto elaborado con cuidado y atención a cada detalle.";
 const descLines=wrapSvgText(desc,34);
 const detail=[];
 if(p.unit)detail.push(`Presentación: ${p.unit}`);
 if(p.category && p.category!=="Otro")detail.push(`Categoría: ${p.category}`);
 if(num(p.timeValue)>0)detail.push(`Elaboración: ${p.timeValue} ${p.timeUnit||"min"}`);
 const safeBiz=ticketText(biz), safeName=ticketText(p.name||"Producto");
 const imageSvg=p.image?`<rect x="560" y="75" width="230" height="145" rx="24" fill="#fff" opacity=".98"/><image href="${p.image}" x="565" y="80" width="220" height="135" preserveAspectRatio="xMidYMid slice" clip-path="inset(0 round 20px)"/>`:"";
 const safeLines=descLines.map(ticketText);
 const safeDetails=detail.slice(0,3).map(ticketText);
 let y=455;
 const descSvg=safeLines.map((x,i)=>`<text x="80" y="${y+i*34}" class="desc">${x}</text>`).join("");
 y+=safeLines.length*34+28;
 const detailsSvg=safeDetails.map((x,i)=>`<text x="80" y="${y+i*30}" class="detail">• ${x}</text>`).join("");
 y+=safeDetails.length*30+44;
 const priceY=Math.max(y+58,700);
 return `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="1100" viewBox="0 0 900 1100">
 <defs><linearGradient id="g" x1="0" x2="1"><stop offset="0" stop-color="#0b9b73"/><stop offset="1" stop-color="#18b889"/></linearGradient></defs>
 <rect width="900" height="1100" fill="#f4f7f9"/>
 <rect x="40" y="35" width="820" height="1030" rx="38" fill="#fff"/>
 <rect x="40" y="35" width="820" height="250" rx="38" fill="url(#g)"/>
 <rect x="40" y="245" width="820" height="40" fill="url(#g)"/>
 <text x="80" y="105" class="biz" fill="#fff">${safeBiz}</text>
 <text x="80" y="145" class="label" fill="#dff8ef">DETALLE DE TU PRODUCTO</text>
 <text x="80" y="215" class="name" fill="#fff">${safeName}</text>
 ${imageSvg}
 <line x1="80" y1="370" x2="820" y2="370" stroke="#dfe7ed" stroke-width="2"/>
 ${descSvg}
 ${detailsSvg}
 <line x1="80" y1="${priceY-45}" x2="820" y2="${priceY-45}" stroke="#dfe7ed" stroke-width="2"/>
 <text x="80" y="${priceY}" class="priceLabel">PRECIO</text>
 <text x="820" y="${priceY+8}" text-anchor="end" class="price">${ticketText(money(price))}</text>
 <text x="450" y="${priceY+105}" text-anchor="middle" class="thanks">Gracias por valorar nuestro trabajo ❤️</text>
 <text x="450" y="${priceY+142}" text-anchor="middle" class="smalltxt">${ticketText(new Date().toLocaleDateString("es-MX",{day:"numeric",month:"long",year:"numeric"}))}</text>
 <style>
 .biz{font:700 42px Arial,sans-serif}.label{font:700 19px Arial,sans-serif;letter-spacing:2px}.name{font:700 52px Arial,sans-serif}
 .desc{font:400 29px Arial,sans-serif;fill:#33485b}.detail{font:600 24px Arial,sans-serif;fill:#627587}
 .priceLabel{font:700 23px Arial,sans-serif;fill:#6c7b8b;letter-spacing:2px}.price{font:700 64px Arial,sans-serif;fill:#0b9b73}
 .thanks{font:700 27px Arial,sans-serif;fill:#10253b}.smalltxt{font:400 20px Arial,sans-serif;fill:#8a98a5}
 </style></svg>`
}
